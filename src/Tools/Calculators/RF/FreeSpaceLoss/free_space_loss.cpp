...
FreeSpaceAttenuationDialog::FreeSpaceAttenuationDialog(QWidget *parent)
    ...
    // Near-field warning footnote
    labelNearFieldWarning = new QLabel("");
    labelNearFieldWarning->setWordWrap(true);
    labelNearFieldWarning->setStyleSheet(
        "color: #990000; font-style: italic; font-size: 11px;");
    labelNearFieldWarning->setVisible(false);
    ...
    mainLayout->addWidget(labelNearFieldWarning);
...
QString FreeSpaceAttenuationDialog::formatDistance(double distanceM) const {
    if (distanceM >= 1e6) {
        return QString::number(distanceM / 1e6, 'g', 3) + " Mm";
    } else if (distanceM >= 1e3) {
        return QString::number(distanceM / 1e3, 'g', 3) + " km";
    } else if (distanceM >= 1.0) {
        return QString::number(distanceM, 'g', 3) + " m";
    } else if (distanceM >= 1e-2) {
        return QString::number(distanceM / 1e-2, 'g', 3) + " cm";
    } else {
        return QString::number(distanceM / 1e-3, 'g', 3) + " mm";
    }
}
...
void FreeSpaceAttenuationDialog::computePathLoss() {
    ...
    // Compute wavelength and far-field threshold
    const double wavelength = SPEED_OF_LIGHT / freqHz;
    const double farFieldThreshold = 10.0 * wavelength;
    ...
    // Track whether any row triggers the near-field warning
    bool anyNearField = false;
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        bool nearField = distances[i] < farFieldThreshold;

        resultsTable->item(i, 1)->setText(QString::number(fspl, 'f', 2));

        if (nearField) {
            // Near-field warning style: red text, yellow background
            resultsTable->item(i, 1)->setForeground(QBrush(QColor(180, 0, 0)));
            resultsTable->item(i, 1)->setBackground(QBrush(QColor(255, 255, 180)));
            anyNearField = true;
        } else if (i == 2) {
            // Highlighted specified distance (not near-field): bold, light yellow
            QFont boldFont;
            boldFont.setBold(true);
            resultsTable->item(i, 1)->setFont(boldFont);
            resultsTable->item(i, 1)->setBackground(QBrush(QColor(255, 255, 200)));
        }
    }

    if (anyNearField) {
        labelNearFieldWarning->setText(
            QString("* The Free Space Loss formula is only valid for far field conditions (d ≥ 10λ = %1). "
                    "\nThe results highlighted in red may be inaccurate.")
                .arg(formatDistance(farFieldThreshold)));
        labelNearFieldWarning->setVisible(true);
    } else {
        labelNearFieldWarning->setVisible(false);
    }
}
